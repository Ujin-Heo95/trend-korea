...
export const PostCard: React.FC<PostCardProps> = React.memo(({ post, rank, isRead, onRead, hasVoted, onVote, style }) => {
  ...
  return (
    <div
      ...
      style={style}
    >
      {/* Clickable content area — community posts open original URL */}
      {(() => {
        ...
        return isCommunity ? (
          <a href={post.url} target="_blank" rel="noopener noreferrer" onClick={() => onRead?.(post.url)} className="flex items-start gap-3 p-4 group">
            {content}
          </a>
        ) : (
          <Link to={`/issue/${post.id}`} onClick={() => onRead?.(post.url)} className="flex items-start gap-3 p-4 group">
            {content}
          </Link>
        );
      })()}
