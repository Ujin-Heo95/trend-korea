  ...
  app.get<{ Querystring: { source?: string; category?: string; subcategory?: string; q?: string; page?: number; limit?: number; sort?: string } }>(
    ...
    async (req) => {
      ...
      if (category) {
        ...
        conditions.push(`(p.category IS NULL OR p.category NOT IN ('movie', 'performance', 'video_popular', 'music', 'travel', 'books', 'ott'))`);
      }
      if (subcategory) {
      ...
      const [rows, count] = await Promise.all([
        app.pg.query(
          `SELECT p.id, p.source_key, p.source_name, p.title, p.url, p.thumbnail,
                  p.author, p.view_count, p.comment_count, p.like_count, p.vote_count, p.published_at, p.first_scraped_at, p.scraped_at, p.category, p.subcategory, p.content_snippet, p.metadata
           FROM posts p
           ${scoreJoin}
           ${where}
           ORDER BY ${orderBy}
