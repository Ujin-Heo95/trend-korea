...
export class DcinsideHotdealScraper extends BaseScraper {
  constructor(pool: Pool) { super(pool); }

  async fetch(): Promise<ScrapedPost[]> {
    const { data } = await axios.get('https://gall.dcinside.com/mini/board/lists/?id=hotdeal', {
      headers: HEADERS,
      timeout: 15000,
    });
